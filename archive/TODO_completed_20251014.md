...
- [x] Debug Ollama 0.12.5 EOF regression bug, downgrade to 0.11.11
- [x] Add timing metrics and --workers CLI argument to stage_4_embed_chunks.py
- [x] Benchmark parallel workers (conclusion: no speedup for real chunks, use workers=1)
- [x] Update `tests/validate_embeddings.py` to use Ollama for queries
- [x] Update all documentation to reflect Ollama migration (768d vectors, $0 cost, version requirements)
- [x] Consolidate performance tests into `test_embedding_performance.py`
- [x] Create `test_data_integrity.py` for database health checks
- [x] Create `test_retrieval_quality.py` for end-to-end RAG validation
- [x] Archive migration-related tests (benchmark_workers, compare_models, compare_openai_vs_ollama)
- [x] Remove duplicate/unreachable code from embeddings.py (simplified embed_chunks method)
- [x] Add timing metrics and performance logging to stage_4_embed_chunks.py
- [x] Add --workers CLI argument to stage_4_embed_chunks.py (default: 1)
- [ ] Re-embed all chunks with Ollama (~55 hours for 1.77M chunks): `docker-compose run --rm -d --name api-embed api bash -c "python -m scripts.stage_4_embed_chunks --workers 1"`
...
- [x] Test Stage 4 implementation (tested Ollama sequential embedding, ~111ms/chunk)
- [x] Test data integrity (orphaned chunks, duplicates, embedding dimensions, NULL fields)
- [x] Test retrieval quality (7 gold-standard Q&A pairs, all passed)
- [x] Test embedding performance (HNSW index, query speed, optimization strategies)
...
- [x] Update all docs for Ollama migration (embedding_service.md, data_design.md, ingestion_pipeline.md, project_spec.md)
- [x] Update docs/cheatsheet.md with long-running embedding job commands
- [x] Add Ollama 0.11.11 version requirements to all relevant docs
...
- [x] Archive migration tests (benchmark_workers, compare_embedding_models, compare_openai_vs_ollama, ollama_migration.md)
- [x] Archive old performance tests (test_index_performance, test_force_index, test_final_performance)
- [x] Remove duplicate/unreachable code from embeddings.py (simplified sequential processing logic)
...
- Background jobs use detached containers for stability
- Stage 1-3 complete: 52K papers collected, fetched, and chunked into 1.89M chunks (717M tokens)
- **Ollama migration complete**: Free embeddings, 768d vectors, ~111ms/chunk sequential
- **Embedding performance**: Parallel workers (4 or 8) provide NO speedup for real chunk sizes
- **Current status**: 3,714 docs embedded (117K chunks), 48,300 docs remaining (~55 hours at 111ms/chunk)
- **Test suite**: 6 active tests (data integrity, retrieval quality, embedding performance, semantic validation)
